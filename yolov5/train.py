...
if __name__ == '__main__':
    ...
    parser.add_argument("--init_method", type=str, default="tcp://192.168.101.37:29500")
    ...
    opt.world_size = opt.world_size #int(os.environ['WORLD_SIZE']) if 'WORLD_SIZE' in os.environ else 1
    opt.global_rank = opt.rank #int(os.environ['RANK']) if 'RANK' in os.environ else -1
    ...
    if opt.local_rank != -1:
        ...
        dist.init_process_group(backend=opt.backend, init_method=opt.init_method, world_size=opt.world_size, rank=opt.rank)  # distributed backend
