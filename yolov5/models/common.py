...
class Shortcat(nn.Module):
    # Add a list of tensors along dimension
    def __init__(self, dimension=1):
        super(Shortcat, self).__init__()
        self.d = dimension
...
class MBConvBlock(nn.Module):
    ...
    def __init__(self, c1, dwc, c2, k=1, s=1, p=None, g=1, act=True):
        super().__init__()

        self.cv1 = Conv(c1, dwc, 1, 1)
        self.cv2 = DWConv(dwc, dwc, k, s)
        self.cv3 = Conv(dwc, c2, 1, 1)

    def forward(self, x):
        return self.cv3(self.cv2(self.cv1(x)))

# class MBConv(nn.Module):
#     def __init__(self, c1, c2, c3, n=1, k=1, s=1, p=None, g=1, act=True):
#         pass